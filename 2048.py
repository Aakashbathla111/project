...
class Game_2048(Frame):
    def __init__(self):
        ...
        self.commands={c.KEY_UP:project_2048.move_up,c.KEY_DOWN:project_2048.move_down,
                        c.KEY_LEFT:project_2048.move_left,c.KEY_RIGHT:project_2048.move_right}
        self.grid_cells=[]
        self.init_grid()
        self.init_matrix()
    ...
    def init_grid(self):
        ...
        for i in range(c.GRID_LEN):
            ...
            for j in range(c.GRID_LEN):
                ...
                t=Label(cell,text="",bg=c.BACKGROUND_COLOR_CELL_EMPTY,justify=CENTER,font=c.FONT,width=5,height=2)
                t.grid()
                grid_row.append(t)
            self.grid_cells.append(grid_row)
    def init_matrix(self):
        self.matrix=project_2048.start_game()
        project_2048.add_2(self.matrix)
        project_2048.add_2(self.matrix)
    ...
    def key_down(self,event):
        ...
        if key in self.commands:
            ...
            if changed:
                project_2048.add_2(self.matrix)
                self.update_grid_cells()
                if project_2048.getstatus(self.matrix)=="Won":
                    self.grid_cells[1][1].configure(text="You",bg=c.BACKGROUND_COLOR_CELL_EMPTY)
                    self.grid_cells[1][2].configure(text="Won",bg=c.BACKGROUND_COLOR_CELL_EMPTY)
                elif project_2048.getstatus(self.matrix)=="Lost":
                    self.grid_cells[1][1].configure(text="You",bg=c.BACKGROUND_COLOR_CELL_EMPTY)
